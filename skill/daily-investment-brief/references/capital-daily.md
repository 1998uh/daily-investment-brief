...
- 同花顺 MCP 负责标的、行情、指数/板块与 ETF 数据；资金流优先使用本机 Eastmoney 适配器，板块请求失败时切换同花顺公开行业资金净额 fallback。
- Eastmoney 提供主力、超大单、大单、中单、小单；同花顺 fallback 只提供流入、流出与净额，报告必须明确标注净额口径，不得伪装为主力分档。
- 5 日与 20 日累计只读取 `data/capital-flow/` 的真实每日缓存，并且只累计与当日相同来源、相同资金口径的数据。历史不足时显示实际天数。
- 非交易日或请求日期与最新数据不一致时，报告同时显示请求日期与实际数据日期。
- Eastmoney 与同花顺 fallback 均因网络、限流、字段变化或空数据失败时，生成带错误说明的报告并返回非零状态；不要给出顺风/逆风结论。
