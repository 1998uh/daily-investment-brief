...
**日期：** 2026-08-06  
**状态：** 已实施（含 Token-Efficient Runtime V1.1）
...
## 20. Token-Efficient Runtime V1.1

2026-08-07 增加模型上下文优化层，不改变三平台门禁、证据规则和最终报告结构。
...
collect
  -> 三平台完整性门禁
  -> prepare
  -> Codex 读取 evidence-tasks.json
  -> 先完成行情、公告和政策取证
  -> verify
  -> Codex 最后读取 article-pack.md
  -> 写 daily-brief.md 和 hypotheses.json
  -> validate
```

`prepare` 不再把完整文章元数据、哈希、假设来源引文和全部历史复制进模型上下文：

- `codex-context.json` 只保留窗口、覆盖率、文章包路径和活跃假设摘要。
- `evidence-tasks.json` 只包含本期到复查时间的假设和查询要求。
- `article-pack.md` 合并正文，只保留文章 ID、平台、作者和标题。
- `article-index.jsonl` 保存 URL、路径、发布时间和源文件哈希，只按最终引用的文章 ID 定向查询。
- `run-manifest.json` 保存运行产物哈希。

### 假设复查策略

新增 `review_policy`：

- `daily`：每次日报复查，默认用于量化假设。
- `weekly`：距上次复查满 7 天后复查，默认用于事件和混合假设。
- `event_triggered`：当日文章命中 `trigger_terms` 时复查。
- `deadline_only`：只在明确 `next_review_date` 或截止日复查，默认用于人工假设。

未到复查时间的假设继续保留在历史账本中，但不进入证据任务、不发起网页搜索，也不写入当期
`verification.json`。截止日始终强制复查。

### 上下文纪律

Skill、日报模板和每日假设契约改为渐进式加载：默认只读取短版运行规则；旧版 LLM 流程、完整
JSON 示例和字段排错文档只在对应场景下读取。大文本文章包必须在所有取证工具调用完成后加载，
避免在后续工具轮次中反复携带全文。
